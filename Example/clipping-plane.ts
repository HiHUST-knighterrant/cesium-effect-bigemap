import { Color, getTimestamp, Viewer } from 'cesium';
...
export const example = (viewer: Viewer, gui: GUI) => {
	_loadTestData();
	BMClippingPlane.enter(viewer, { edgeWidth: 5, edgeColor: Color.RED });

	const options = {
		batch: false,
		draw: BMClippingPlane.draw,
		removeAll: BMClippingPlane.removeAll,
	};
	...
	folder
		...
		.onChange(async v => {
			...
			batch_index !== undefined && (await BMClippingPlane.remove(batch_index));
			batch_index = undefined;
			if (v) {
				batch_index = await BMClippingPlane.draw(positions);
				if (ctime !== batch_time) {
					await BMClippingPlane.remove(batch_index);
					batch_index = undefined;
				}
			}
		});
