import { BoxGeometry, CameraEventType, Cartesian2, Cartesian3, Cartographic, Cesium3DTileset, Cesium3DTileStyle, CesiumTerrainProvider, CircleEmitter, ClippingPlane, ClippingPlaneCollection, Color, createWorldTerrain, defined, Ellipsoid, EventHelper, HeadingPitchRoll, Ion, IonResource, JulianDate, Math, Matrix3, Matrix4, ParticleSystem, Plane, PostProcessStage, PostProcessStageComposite, PostProcessStageLibrary, Quaternion, ScreenSpaceEventHandler, ScreenSpaceEventType, SkyBox, SphereEmitter, Transforms, TranslationRotationScale, UrlTemplateImageryProvider, VertexFormat, Viewer, MultiClippingPlaneCollection, Ray } from "cesium";
...
Ion.defaultAccessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJlN2MzMWE5ZC0wYzkyLTQ3ODMtYmJlYy1iN2QxMWI4NjU3ODUiLCJpZCI6OTMzMjYsImlhdCI6MTY1MjI1MzYyN30.irrMfifWXXSjF_wHoeyjgkmjDHZ4LBnFL4hIZf-HSGg";
...
let tileset: Cesium3DTileset = viewer.scene.primitives.add(new Cesium3DTileset({
...
function update3dtiles(tileset: Cesium3DTileset, opt: any) {
  ...
  // 建立从局部到世界的坐标矩阵
  Matrix4.multiplyByUniformScale(mtx, opt.scale,
    mtx);
...
option.defaultResetView = new Cartographic(Math.toRadians(111.50623801848565), Math.toRadians(2.8997206760441205), 8213979.400955964)
//相机方向
option.orientation = {
  ...
  handler.setInputAction(function (evt: any) {
    ...
    if (defined(cartesian)) {
      ...
      firedata = new ParticleSystem({
        ...
        modelMatrix: Transforms.eastNorthUpToFixedFrame(Cartesian3.fromDegrees(lng, lat,
          height)), //从模型转化成世界坐标
