import { defaultValue, PostProcessStage, Viewer } from "cesium";
...
export class Rain {
    ...
    async init() {
        this.rainStage = new PostProcessStage({
            name: 'czm_rain',
            fragmentShader: await this.rain1(),
            uniforms: {
                tiltAngle: () => {
    ...
    rain1() {
        const FRAGMENTSHADER = "src/Rain1.fs";
        const FSPROMISE = fetch(FRAGMENTSHADER).then(resp => resp.text()
        ).then(shader => {
            return shader;
        });
        return FSPROMISE;
