...
const _updatePoint = (
	...
	let cartesian = _viewer!.scene.globe.pick(ray!, _viewer!.scene);
	...
	if (_positions.length >= 2) {
		...
		for (let i = 0; i < _projection_positions.length - 1; i++) {
			...
			if (i === _projection_positions.length - 2) {
				const collineation = _isSegmentsCollineation(
					check_line[0],
					check_line[1],
					passivity_line[0],
					passivity_line[1]
	...
	if (!is_move && effective && cartesian !== _positions[0] && cartesian_web_mercator !== _projection_positions[0]) {
...
const _clearDrawLine = (_draw_line: Array<Entity>) => {
	_draw_line.forEach(
		v => _viewer!.entities.remove(v)
	); 
};
...
export const draw = async () => {
	return new Promise<_PROJECTION_ID>((resolve, reject) => {
		...
		_handler!.setInputAction(function (event: any) {
			const _r = _updatePoint(_positions, _projection_positions, _draw_line, event.position, false, _delete_flag);
			_delete_flag = !_r.effective;
			_r.done && _done(_positions, _projection_positions, _riding_index, resolve, reject, _draw_line);
		}, ScreenSpaceEventType.LEFT_CLICK);

		_handler!.setInputAction(function (event: any) {
			// console.log(event);
			_delete_flag = _updatePoint(
				_positions,
...
const _done = async (
	...
	if (_positions.length >= 3) {
			...
			_ploygons_boundingbox_backup.forEach((boundingbox, i) => {
				...
				polygonA.forEach((va, ia) => {
					...
					polygonB.forEach((vb, ib) => {
						...
						!ia &&
							_pointInsidePolygon(vb, polygonA) &&
		...
		_projection2BottomSurface[_projection_index] =
			_bottomSurfaces.push(_createBottomSurface(positions.position_bottom_surface)) - 1;
		const create_wall_index =
			_wall.push(_createWellWall(positions.position, positions.min_heights, positions.max_heights)) - 1;
...
export const remove = async (_projection_id: _PROJECTION_ID) => {
	...
	const check_intersection_lines = [..._intersection_line].filter(
		(_, i) =>
	...
	Object.keys(wall_obj).forEach(async v => {
		...
		_projection2WallIndex[key].push(
			_wall.push(_createWellWall(wall_obj[key].position, wall_obj[key].min_heights, wall_obj[key].max_heights)) -
				1
		);
	});
};
...
const _isSegmentsCollineation = (a: Cartesian2, b: Cartesian2, c: Cartesian2, d: Cartesian2) => {
	...
	if (
		window.Math.abs(
			Cartesian2.cross(Cartesian2.subtract(b, a, new Cartesian2()), Cartesian2.subtract(d, c, new Cartesian2()))
		) > 0.00001
	)
		return [[a, b]];
	if (
		window.Math.abs(
			Cartesian2.cross(Cartesian2.subtract(c, a, new Cartesian2()), Cartesian2.subtract(d, a, new Cartesian2()))
		) > 0.00001
	)
		return [[a, b]];
	...
	if (polygon.length === 2) {
		...
		for (let ii = 0; ii < _intersection_line.length; ii++) {
			...
			seg_backup.forEach(v => {
				_isSegmentsCollineation(v[0], v[1], _intersection_line[ii][0], _intersection_line[ii][1]).forEach(vv =>
					seg.push(vv)
				);
			});
			if (!seg) break;
		...
		for (let i = 0; i < polygon.length; i++) {
			...
			for (let ii = 0; ii < _intersection_line.length; ii++) {
				...
				seg = [];
				seg_backup.forEach(v => {
					_isSegmentsCollineation(v[0], v[1], _intersection_line[ii][0], _intersection_line[ii][1]).forEach(
						vv => seg.push(vv)
					);
				});
				if (!seg) break;
...
const _createWellWall = function (position: Cartesian3[][], min_heights: number[][], max_heights: number[][]) {
	const geometryInstances = position.map(
		(v, i) =>
...
const _drawDebugLine = (v: [Cartesian2, Cartesian2] | [Cartesian3, Cartesian3], color: Color, dotted: boolean) => {
	let entity: Entity;
...
const _drawIntersectionLine = (
	...
	line.forEach((v, i) => {
		if (v instanceof Cartesian3) {
			...
			v && _drawDebugLine(v, color, dotted);
		}
	});
	return entity!;
};
